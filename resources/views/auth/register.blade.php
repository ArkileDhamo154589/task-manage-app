         ...
         <div class="card-body">
            <form action="{{route('register')}}" method="POST">
               ...
               <div class="form-group my-1">
                  ...
                  {{-- Email validation error starts --}}
                    @error('email')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{$message}}</strong>
                        </span>
                    @enderror
                  {{-- Email validation error ends using boodstrap classes and laravel error methods --}}
               </div>
               <div class="form-group my-1">
                  <label for="name">Name</label>
                  <input type="text" name="name" class="form-control @error('email') is-invalid @enderror" value="{{old('name')}}" placeholder="Username" required>
                    @error('name')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{$message}}</strong>
                        </span>
